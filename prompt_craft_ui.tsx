...
import { Zap, Code, Feather, BookOpen, Brain, Copy, Check, Settings, Info, ChevronDown, Sparkles, Eye, EyeOff, AlertCircle, Loader } from 'lucide-react';
...
const PromptCraftUI = () => {
  ...
  const [animateOutput, setAnimateOutput] = useState(true);
  ...
  const enhancePromptAPI = async (userInput, template = null, model = 'default') => {
    try {
  ...
  useEffect(() => {
    ...
    if (autoEnhance && userInput.length > 10 && apiConnected) {
      const timer = setTimeout(() => {
        handleEnhance();
      }, 1500);
      return () => clearTimeout(timer);
    }
  }, [userInput, autoEnhance, apiConnected]);
  ...
  const handleEnhance = async () => {
    if (!userInput.trim() || !apiConnected) return;
  ...
  const TemplateIcon = config.templates[selectedTemplate] ? 
    iconMap[selectedTemplate] || Brain : 
    Brain;
    ...
    return (
      <div className="min-h-screen bg-black text-cyan-400 p-8 font-mono flex items-center justify-center">
        <div className="text-center">
          <Loader className="w-12 h-12 animate-spin mx-auto mb-4 text-pink-500" />
          <h2 className="text-2xl font-bold mb-2">Initializing PromptCraft</h2>
          <p className="text-sm text-green-400">Connecting to API server...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-cyan-400 p-8 font-mono overflow-hidden relative">
      {/* Error Banner */}
      {error && (
        <div className="fixed top-4 left-4 right-4 z-50 bg-red-900/80 border border-red-500 rounded-lg p-4 flex items-center space-x-3">
      ...
      <div className="fixed top-4 right-4 z-40 flex items-center space-x-2 text-xs">
      ...
      <div className="relative z-10 mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-5xl font-bold neon-text bg-gradient-to-r from-cyan-400 via-pink-500 to-green-400 bg-clip-text text-transparent">
              PromptCraft
            </h1>
            <div className="text-xs text-pink-400 animate-pulse">v3.0.1</div>
          </div>
          <button
            onClick={() => setShowSettings(!showSettings)}
            className="glass-morph p-3 rounded-lg hover:bg-cyan-900/30 transition-all duration-300 group"
            title="Settings"
          >
            <Settings className={`w-6 h-6 transition-transform duration-500 ${showSettings ? 'rotate-180' : ''} group-hover:text-pink-400`} />
          </button>
        </div>
        <p className="text-sm text-green-400 mt-2 ml-14">Neural Prompt Enhancement System</p>
      </div>

      {/* Settings Panel */}
      {showSettings && (
        <div className="relative z-10 mb-6 glass-morph p-6 rounded-lg border-pink-500/30 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className="flex items-center space-x-3 cursor-pointer group">
              <input
                type="checkbox"
                checked={animateOutput}
                onChange={(e) => setAnimateOutput(e.target.checked)}
                className="w-5 h-5 accent-cyan-400"
              />
              <span className="text-sm group-hover:text-cyan-300">Animate Output</span>
            </label>
            <label className="flex items-center space-x-3 cursor-pointer group">
              <input
                type="checkbox"
              ...
              />
              <span className="text-sm group-hover:text-pink-300">Show Tooltips</span>
            </label>
            <label className="flex items-center space-x-3 cursor-pointer group">
              <input
                type="checkbox"
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="relative">
          <label className="block text-sm font-semibold text-cyan-400 mb-2 flex items-center">
            <TemplateIcon className="w-4 h-4 mr-2" />
            Template Mode
            {showTooltips && (
              <Info className="w-4 h-4 ml-2 text-pink-400 cursor-help" title="Select the type of prompt enhancement" />
            )}
          </label>
          <div className="relative">
            <button
              onClick={() => setShowTemplateDropdown(!showTemplateDropdown)}
              className="w-full glass-morph p-4 rounded-lg hover:bg-cyan-900/30 transition-all duration-300 flex items-center justify-between group"
            >
              <div className="flex items-center space-x-3">
                <TemplateIcon className="w-5 h-5 text-pink-400" />
                <span className="group-hover:text-cyan-300">
                  {config.templates[selectedTemplate]?.name || 'Loading...'}
                </span>
              </div>
              <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${showTemplateDropdown ? 'rotate-180' : ''}`} />
            </button>
            {showTemplateDropdown && (
              <div className="absolute top-full mt-2 w-full glass-morph rounded-lg overflow-hidden border border-cyan-500/30 z-20">
                {Object.entries(config.templates).map(([key, template]) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-4 flex items-center space-x-3 hover:bg-cyan-900/50 transition-all duration-200 ${
                        selectedTemplate === key ? 'bg-cyan-900/30 border-l-4 border-pink-500' : ''
                      }`}
                    >
        ...
        <div className="relative">
          <label className="block text-sm font-semibold text-cyan-400 mb-2 flex items-center">
            ...
            Target Model
            {showTooltips && (
              <Info className="w-4 h-4 ml-2 text-pink-400 cursor-help" title="Optimize prompt for specific AI model" />
            )}
          </label>
          <div className="relative">
            <button
            ...
            >
              ...
              <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${showModelDropdown ? 'rotate-180' : ''}`} />
            </button>
            {showModelDropdown && (
              <div className="absolute top-full mt-2 w-full glass-morph rounded-lg overflow-hidden border border-cyan-500/30 z-20">
                {models.map((model) => (
                  <button
                    ...
                    className={`w-full p-4 flex items-center justify-between hover:bg-cyan-900/50 transition-all duration-200 ${
                      selectedModel === model.id ? 'bg-cyan-900/30 border-l-4 border-pink-500' : ''
                    }`}
                  >
      ...
      <div className="relative z-10 mb-6">
        <label className="block text-sm font-semibold text-cyan-400 mb-2 flex items-center justify-between">
          <span>Input Prompt</span>
          <span className="text-xs text-pink-400">{charCount} characters</span>
        </label>
        <div className="holographic-border rounded-lg p-1">
          <textarea
            ref={inputRef}
            value={userInput}
            onChange={(e) => setUserInput(e.target.value)}
            placeholder="Enter your prompt here... The system will analyze and enhance it."
            className="w-full h-40 bg-black/80 text-green-400 p-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-none placeholder-green-800 font-mono"
      ...
      <div className="relative z-10 mb-6 flex justify-center">
        <div className="button-container">
          <div
            className="button"
            onClick={() => {
              if (!userInput.trim() || isProcessing || !apiConnected) return;
              handleEnhance();
            }}
            style={{
                cursor: (!userInput.trim() || isProcessing || !apiConnected) ? 'not-allowed' : 'pointer',
                opacity: (!userInput.trim() || isProcessing || !apiConnected) ? 0.6 : 1
            }}
          >
      ...
      {/* Output Field */}
      {enhancedPrompt && (
        <div className={`relative z-10 ${animateOutput ? 'animate-in fade-in slide-in-from-bottom-4 duration-500' : ''}`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              onClick={handleCopy}
              className="glass-morph px-4 py-2 rounded-lg hover:bg-pink-900/30 transition-all duration-300 flex items-center space-x-2 group"
            >
              {copied ? (
                <>
                ...
                <>
                  <Copy className="w-4 h-4 group-hover:text-pink-400" />
                  <span className="group-hover:text-pink-400">Copy</span>
                </>
              )}
            </button>
          </div>
          <div className="holographic-border rounded-lg p-1">
            <div
              ref={outputRef}
              className="bg-black/80 text-pink-400 p-6 rounded-lg h-96 overflow-y-auto font-mono text-sm whitespace-pre-wrap"
              style={{ 
                scrollbarWidth: 'thin',
                scrollbarColor: '#ec4899 #000000'
              }}
            >
              {enhancedPrompt}
            </div>
      ...
      {/* Footer */}
      <div className="relative z-10 mt-8 text-center text-xs text-cyan-600">
